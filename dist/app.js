...
const seaDebugDefaults={medX:0,medY:0,redX:0,redY:0};
const seaDebugState={...seaDebugDefaults};
function seaDebugValues(){return{medX:Number(dom.seaDebugMedX?.value)||0,medY:Number(dom.seaDebugMedY?.value)||0,redX:Number(dom.seaDebugRedX?.value)||0,redY:Number(dom.seaDebugRedY?.value)||0}}
function applySeaDebug(){const values={...seaDebugState};[[dom.mediterranean,values.medX,values.medY],[dom.redSea,values.redX,values.redY]].forEach(([node,x,y])=>{if(!node)return;node.style.setProperty("--sea-debug-x",`${x}px`);node.style.setProperty("--sea-debug-y",`${y}px`)})}
 function applyFlagDebugSettings(id){const settings={...flagDebugSettings(id),...(flagDebugDrafts[id]||{})};activeCountries().filter(path=>path.dataset.id===id).forEach(path=>{if(settings.color)path.style.setProperty("--polity-color",settings.color)});document.querySelectorAll(`.country-flag[data-for="${id}"] image`).forEach(image=>{const b={x:Number(image.dataset.baseX),y:Number(image.dataset.baseY),width:Number(image.dataset.baseWidth),height:Number(image.dataset.baseHeight)};const cx=b.x+b.width/2,cy=b.y+b.height/2,sx=Number(settings.scaleX)||1,sy=Number(settings.scaleY)||1,ox=Number(settings.offsetX)||0,oy=Number(settings.offsetY)||0;image.setAttribute("transform",`translate(${cx+ox} ${cy+oy}) scale(${sx} ${sy}) translate(${-cx} ${-cy})`)});document.querySelectorAll(`.country-flag[data-for="${id}"] .flag-fill`).forEach(fill=>fill.setAttribute("fill",settings.flagFill||"transparent"))}
...
function historicalStyles(y){
  ...
  countries.forEach((path,index)=>{const polity=polities[index];path.dataset.polity=polity;path.style.setProperty("--polity-color",colors.get(polity));path.classList.toggle("dependent",dependentIds.includes(path.dataset.id));path.classList.toggle("context",["hejaz","shammar","najd"].includes(path.dataset.id))});
}
...
function debugEntityIds(){return [...new Set(activeCountries().map(path=>path.dataset.id))]}
 ...
 function loadDebugForm(id){const s={...flagDebugSettings(id),...(flagDebugDrafts[id]||{})},path=activeCountries().find(candidate=>candidate.dataset.id===id),defaultColor=path?.style.getPropertyValue("--polity-color").trim()||"#a44e3b";dom.flagDebugScaleX.value=s.scaleX;dom.flagDebugScaleY.value=s.scaleY;dom.flagDebugOffsetX.value=s.offsetX;dom.flagDebugOffsetY.value=s.offsetY;dom.flagDebugColor.value=s.color||defaultColor;dom.flagDebugFill.value=s.flagFill||defaultColor;updateDebugOutputs();applyFlagDebugSettings(id)}
function populateDebugEntities(){const y=Number(dom.year.value),ids=debugEntityIds();dom.flagDebugCountry.innerHTML=ids.map(id=>`<option value="${id}">${displayNameAt(id,y)}</option>`).join("");if(selected&&ids.includes(selected))dom.flagDebugCountry.value=selected;loadDebugForm(dom.flagDebugCountry.value)}
function refreshDebugDraft(){const id=dom.flagDebugCountry.value;if(!id)return;flagDebugDrafts[id]=debugFormValues();updateDebugOutputs();applyFlagDebugSettings(id);dom.flagDebugStatus.textContent="实时预览中；点击“保存到浏览器”后将在本机保留。"}
function saveDebugSettings(){const id=dom.flagDebugCountry.value;if(!id)return;const saved=readFlagDebug();saved[id]={...flagDebugSettings(id),...debugFormValues()};try{localStorage.setItem(FLAG_DEBUG_STORAGE,JSON.stringify(saved,null,2));flagDebugDrafts[id]={...saved[id]};dom.flagDebugStatus.textContent="已保存到本浏览器。"}catch{dom.flagDebugStatus.textContent="浏览器存储不可用，请使用导出 JSON。"}applyFlagDebugSettings(id)}
function resetDebugSettings(){const id=dom.flagDebugCountry.value;if(!id)return;delete flagDebugDrafts[id];const saved=readFlagDebug();delete saved[id];try{localStorage.setItem(FLAG_DEBUG_STORAGE,JSON.stringify(saved))}catch{}historicalStyles(Number(dom.year.value));syncFlags(Number(dom.year.value));loadDebugForm(id);dom.flagDebugStatus.textContent="已恢复默认设置。"}
function exportDebugSettings(){const payload=JSON.stringify(readFlagDebug(),null,2),blob=new Blob([payload],{type:"application/json"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download="mena-flag-debug.json";a.click();setTimeout(()=>URL.revokeObjectURL(url),0);dom.flagDebugStatus.textContent="已导出 mena-flag-debug.json。"}
 if(dom.flagDebugButton){dom.flagDebugButton.addEventListener("click",()=>{populateDebugEntities();dom.flagDebugDialog.showModal()});dom.flagDebugClose.addEventListener("click",()=>dom.flagDebugDialog.close());dom.flagDebugDialog.addEventListener("click",e=>{if(e.target===dom.flagDebugDialog)dom.flagDebugDialog.close()});dom.flagDebugCountry.addEventListener("change",()=>loadDebugForm(dom.flagDebugCountry.value));[dom.flagDebugScaleX,dom.flagDebugScaleY,dom.flagDebugOffsetX,dom.flagDebugOffsetY,dom.flagDebugColor,dom.flagDebugFill].forEach(input=>input.addEventListener("input",refreshDebugDraft));dom.flagDebugSave.addEventListener("click",saveDebugSettings);dom.flagDebugReset.addEventListener("click",resetDebugSettings);dom.flagDebugExport.addEventListener("click",exportDebugSettings)}
 if(dom.seaDebugMedX){const seaInputs=[dom.seaDebugMedX,dom.seaDebugMedY,dom.seaDebugRedX,dom.seaDebugRedY];seaInputs.forEach(input=>input.addEventListener("input",()=>{Object.assign(seaDebugState,seaDebugValues());seaInputs.forEach(control=>{const output=document.querySelector(`#${control.id}Value`);if(output)output.textContent=control.value});applySeaDebug();if(dom.flagDebugStatus)dom.flagDebugStatus.textContent="海域标注仅在当前页面会话中预览。"}));dom.seaDebugReset?.addEventListener("click",()=>{Object.assign(seaDebugState,seaDebugDefaults);seaInputs.forEach(input=>{input.value=0;const output=document.querySelector(`#${input.id}Value`);if(output)output.textContent="0"});applySeaDebug();if(dom.flagDebugStatus)dom.flagDebugStatus.textContent="海域标注已恢复默认位置。"});applySeaDebug()}
