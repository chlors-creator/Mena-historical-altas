...
function syncFlags(y){
  ...
  activeCountries().forEach(path=>{const b=path.getBBox();if(!Number.isFinite(b.x)||!Number.isFinite(b.y))return;const w=Math.max(18,Math.min(38,b.width*.58||18)),h=Math.max(9,Math.min(18,b.height*.28||9));const flag=document.createElementNS(ns,"g");flag.classList.add("country-flag");flag.dataset.for=path.dataset.id;flag.setAttribute("transform",`translate(${nflag(b.x+b.width/2)} ${nflag(b.y+b.height/2)})`);flag.setAttribute("aria-hidden","true");flag.innerHTML=flagSvg(path.dataset.id,y,w,h);root.append(flag);if(document.activeElement===path)setHoverState(path.dataset.id,true)})
}
