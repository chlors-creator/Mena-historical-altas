...
const BOUNDARY_DEBUG_STORAGE="menaBoundaryDebugV1";
...
function boundaryDebugReadStore(){try{const parsed=JSON.parse(localStorage.getItem(BOUNDARY_DEBUG_STORAGE)||"{}");return parsed&&typeof parsed==="object"?parsed:{}}catch{return{}}}
...
function boundaryDebugStoreValue(){return{version:1,updatedAt:new Date().toISOString(),year:Number(dom.year.value),transforms:boundaryDebugTransforms,imported:boundaryDebugImported,saved:boundaryDebugSaved}}
function boundaryDebugPersist(){try{localStorage.setItem(BOUNDARY_DEBUG_STORAGE,JSON.stringify(boundaryDebugStoreValue()))}catch(error){return false}return true}
function boundaryDebugId(prefix="boundary"){return `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2,8)}`}
function boundaryDebugName(id,y){return id==="ottoman"?"奥斯曼帝国":id==="yemen-south"?(y<1967?"亚丁保护国":"南也门"):meta[id]?.[0]||id}
function boundaryDebugTargetMeta(value=boundaryDebugTarget){const [type,...rest]=String(value||"").split(":");const id=rest.join(":");if(!type||!id)return null;return{type,id,value}}
function boundaryDebugStorageKey(target,y=Number(dom.year.value)){if(!target)return"";return target.type==="current"?`${y}::${target.id}`:target.id}
function boundaryDebugSettings(target,y=Number(dom.year.value)){const value=target?.value||boundaryDebugTarget,key=boundaryDebugStorageKey(target,y),item=boundaryDebugItemFor(target);return{...boundaryDebugDefaults,...(item?.transform||{}),...(boundaryDebugTransforms[key]||{}),...(boundaryDebugDrafts[value]||{})}}
function boundaryDebugHasCurrentOverride(target,y){if(!target)return false;const key=boundaryDebugStorageKey(target,y);return Object.prototype.hasOwnProperty.call(boundaryDebugTransforms,key)||Object.prototype.hasOwnProperty.call(boundaryDebugDrafts,target.value)}
...
function boundaryDebugWriteForm(settings){const s={...boundaryDebugDefaults,...settings};dom.boundaryDebugScaleX.value=s.scaleX;dom.boundaryDebugScaleY.value=s.scaleY;dom.boundaryDebugOffsetX.value=s.offsetX;dom.boundaryDebugOffsetY.value=s.offsetY;dom.boundaryDebugRotation.value=s.rotation;dom.boundaryDebugOpacity.value=s.opacity;boundaryDebugUpdateOutputs()}
function boundaryDebugUpdateOutputs(){const pairs=[[dom.boundaryDebugScaleX,document.querySelector("#boundaryDebugScaleXValue"),v=>`${Number(v).toFixed(2)}×`],[dom.boundaryDebugScaleY,document.querySelector("#boundaryDebugScaleYValue"),v=>`${Number(v).toFixed(2)}×`],[dom.boundaryDebugOffsetX,document.querySelector("#boundaryDebugOffsetXValue"),v=>String(v)],[dom.boundaryDebugOffsetY,document.querySelector("#boundaryDebugOffsetYValue"),v=>String(v)],[dom.boundaryDebugRotation,document.querySelector("#boundaryDebugRotationValue"),v=>`${Number(v).toFixed(1)}°`],[dom.boundaryDebugOpacity,document.querySelector("#boundaryDebugOpacityValue"),v=>`${Math.round(Number(v)*100)}%`]];pairs.forEach(([input,out,format])=>{if(input&&out)out.textContent=format(input.value)})}
function boundaryDebugPathRecordsBounds(records){const boxes=records.map(record=>record?.d?pathDataBox(record.d):null).filter(box=>box&&Number.isFinite(box.x));if(!boxes.length)return{x:0,y:0,width:0,height:0};const left=Math.min(...boxes.map(box=>box.x)),top=Math.min(...boxes.map(box=>box.y)),right=Math.max(...boxes.map(box=>box.x+box.width)),bottom=Math.max(...boxes.map(box=>box.y+box.height));return{x:left,y:top,width:right-left,height:bottom-top}}
function boundaryDebugElementBounds(elements){const boxes=[];elements.forEach(element=>{try{const box=element.getBBox();if(box.width||box.height)boxes.push({x:box.x,y:box.y,width:box.width,height:box.height})}catch{}});if(!boxes.length)return{x:0,y:0,width:0,height:0};const left=Math.min(...boxes.map(box=>box.x)),top=Math.min(...boxes.map(box=>box.y)),right=Math.max(...boxes.map(box=>box.x+box.width)),bottom=Math.max(...boxes.map(box=>box.y+box.height));return{x:left,y:top,width:right-left,height:bottom-top}}
function boundaryDebugTransform(anchor,settings){const cx=anchor.x+anchor.width/2,cy=anchor.y+anchor.height/2,sx=Number(settings.scaleX)||1,sy=Number(settings.scaleY)||1,ox=Number(settings.offsetX)||0,oy=Number(settings.offsetY)||0,rotation=Number(settings.rotation)||0;return`translate(${cx+ox} ${cy+oy}) rotate(${rotation}) scale(${sx} ${sy}) translate(${-cx} ${-cy})`}
function boundaryDebugResetElement(element){if(!element)return;if(element.dataset.boundaryDebugOriginalTransform===undefined){element.dataset.boundaryDebugOriginalTransform=element.getAttribute("transform")||""}const original=element.dataset.boundaryDebugOriginalTransform;if(original)element.setAttribute("transform",original);else element.removeAttribute("transform");if(element.dataset.boundaryDebugOriginalOpacity!==undefined)element.style.opacity=element.dataset.boundaryDebugOriginalOpacity}
function boundaryDebugApplyElement(element,anchor,settings,opacity){if(!element)return;if(element.dataset.boundaryDebugOriginalTransform===undefined)element.dataset.boundaryDebugOriginalTransform=element.getAttribute("transform")||"";if(element.dataset.boundaryDebugOriginalOpacity===undefined)element.dataset.boundaryDebugOriginalOpacity=element.style.opacity||"";element.setAttribute("transform",boundaryDebugTransform(anchor,settings));if(opacity!==undefined)element.style.opacity=String(opacity)}
function boundaryDebugProjectPoint(point){const lon=Number(point?.[0]),lat=Math.max(-85,Math.min(85,Number(point?.[1])));if(!Number.isFinite(lon)||!Number.isFinite(lat))return null;const west=-18,east=63,north=43,south=7,left=58,right=1040,top=55,bottom=530;const x=left+(lon-west)/(east-west)*(right-left),mercator=v=>Math.log(Math.tan(Math.PI/4+v*Math.PI/360)),northY=mercator(north),southY=mercator(south),y=top+(northY-mercator(lat))/(northY-southY)*(bottom-top);return[x,y]}
function boundaryDebugRingPath(ring){if(!Array.isArray(ring)||ring.length<3)return"";const points=ring.map(boundaryDebugProjectPoint).filter(Boolean);if(points.length<3)return"";return points.map((point,index)=>`${index?"L":"M"}${point[0].toFixed(2)},${point[1].toFixed(2)}`).join("")+"Z"}
function boundaryDebugGeometryPaths(geometry){if(!geometry)return[];if(geometry.type==="Polygon")return(geometry.coordinates||[]).map(boundaryDebugRingPath).filter(Boolean).map(d=>({d,fillRule:"evenodd"}));if(geometry.type==="MultiPolygon")return(geometry.coordinates||[]).flatMap(polygon=>boundaryDebugGeometryPaths({type:"Polygon",coordinates:polygon}));if(geometry.type==="GeometryCollection")return(geometry.geometries||[]).flatMap(boundaryDebugGeometryPaths);return[]}
function boundaryDebugFeatureItem(feature,index,prefix="imported"){const props=feature.properties||{},id=String(feature.id||props.id||props.ID||props.iso_a3||props.name||props.name_zh||`${prefix}-${index}`),name=String(props.name_zh||props.name||props.NAME||id),paths=boundaryDebugGeometryPaths(feature.geometry);return paths.length?{key:boundaryDebugId(prefix),id,name,year:null,source:String(props.source||props.source_url||"GeoJSON 导入"),paths}:null}
function boundaryDebugParseSvg(text){const doc=new DOMParser().parseFromString(text,"image/svg+xml"),paths=[...doc.querySelectorAll("path[d]")];return paths.map((path,index)=>({d:path.getAttribute("d")||"",fillRule:path.getAttribute("fill-rule")||"nonzero",label:path.getAttribute("data-id")||path.getAttribute("id")||path.getAttribute("aria-label")||`SVG 路径 ${index+1}`})).filter(record=>record.d).reduce((items,record,index)=>{const existing=items.find(item=>item.name===record.label);if(existing)existing.paths.push(record);else items.push({key:boundaryDebugId("imported"),id:`svg-${index+1}`,name:record.label,year:null,source:"SVG 路径导入",paths:[record]});return items},[])}
function boundaryDebugParseJson(value){
  if(value?.version&&Array.isArray(value.imported)){boundaryDebugLoadStore(value);return{config:true,items:[]}}
  if(["Polygon","MultiPolygon","GeometryCollection"].includes(value?.type))return{config:false,items:[boundaryDebugFeatureItem({type:"Feature",properties:{},geometry:value},0,"geometry")].filter(Boolean)};
  const features=value?.type==="FeatureCollection"?value.features:value?.type==="Feature"?[value]:null;
  if(Array.isArray(features))return{config:false,items:features.map((feature,index)=>boundaryDebugFeatureItem(feature,index)).filter(Boolean)};
  const records=Array.isArray(value)?value:[value];
  const items=records.map((item,index)=>{
    const paths=Array.isArray(item?.paths)
      ? item.paths.map(path=>typeof path==="string"?{d:path}:{d:path?.d||path?.path,fillRule:path?.fillRule||"nonzero"}).filter(path=>path.d)
      : (item?.path||item?.d)?[{d:item.path||item.d,fillRule:item.fillRule||"nonzero"}]:[];
    return paths.length?{key:boundaryDebugId("imported"),id:String(item.id||`json-${index+1}`),name:String(item.name||item.name_zh||item.id||`JSON 路径 ${index+1}`),year:item.year??null,source:String(item.source||"JSON 路径导入"),paths}:null;
  }).filter(Boolean);
  return{config:false,items};
}
function boundaryDebugParseImport(text,fileName){const extension=String(fileName||"").toLowerCase().split(".").pop();if(extension==="svg"||/\<svg[\s\S]*\<path/i.test(text))return{config:false,items:boundaryDebugParseSvg(text)};let value;try{const source=extension==="js"?text.slice(text.indexOf("["),text.lastIndexOf("]")+1):text;value=JSON.parse(source)}catch(error){throw new Error("文件不是有效的 SVG、GeoJSON、JSON 或地图数据 JS 文件")};return boundaryDebugParseJson(value)}
...
function boundaryDebugPopulateTargets(preserve=true){if(!dom.boundaryDebugTarget)return;const previous=preserve?(dom.boundaryDebugTarget.value||boundaryDebugTarget):"",y=Number(dom.year.value),currentIds=[...new Set(activeCountries().map(path=>path.dataset.id))],options=[];const addGroup=(label,entries)=>{if(!entries.length)return;const group=document.createElement("optgroup");group.label=label;entries.forEach(entry=>{const option=document.createElement("option");option.value=entry.value;option.textContent=entry.label;group.append(option)});dom.boundaryDebugTarget.append(group)};addGroup(`当前地图 · ${y}`,currentIds.map(id=>({value:`current:${id}`,label:boundaryDebugName(id,y)})));addGroup("已导入疆域",boundaryDebugImported.map(item=>({value:`imported:${item.key}`,label:`${item.name||item.id} · 导入`})));addGroup("已保存快照",boundaryDebugSaved.map(item=>({value:`saved:${item.key}`,label:`${item.name||item.id} · 快照`})));const valid=[...dom.boundaryDebugTarget.options].find(option=>option.value===previous)||[...dom.boundaryDebugTarget.options].find(option=>option.value===boundaryDebugTarget)||dom.boundaryDebugTarget.options[0];if(valid){dom.boundaryDebugTarget.value=valid.value;boundaryDebugTarget=valid.value;boundaryDebugLoadForm()}}
function boundaryDebugLoadForm(){const target=boundaryDebugTargetMeta(dom.boundaryDebugTarget?.value||boundaryDebugTarget);if(!target){if(dom.boundaryDebugCurrent)dom.boundaryDebugCurrent.textContent=`当前年份：${dom.year.value} · 没有可调试对象`;return}boundaryDebugTarget=target.value;const settings=boundaryDebugSettings(target);if(target.type==="current"&&!boundaryDebugHasCurrentOverride(target,Number(dom.year.value)))settings.opacity=1;boundaryDebugWriteForm(settings);const item=boundaryDebugItemFor(target),name=target.type==="current"?boundaryDebugName(target.id,Number(dom.year.value)):item?.name||item?.id||target.id;if(dom.boundaryDebugCurrent)dom.boundaryDebugCurrent.textContent=`${Number(dom.year.value)} · ${name} · ${target.type==="current"?"现有疆域":target.type==="imported"?"导入预览":"已保存快照"}`;boundaryDebugRenderLayer(Number(dom.year.value))}
function boundaryDebugRefreshDraft(){const target=boundaryDebugTargetMeta(dom.boundaryDebugTarget?.value||boundaryDebugTarget);if(!target)return;boundaryDebugTarget=target.value;boundaryDebugDrafts[target.value]=boundaryDebugReadForm();boundaryDebugApplyTransforms(Number(dom.year.value));boundaryDebugLoadForm()}
function boundaryDebugCommit(){const target=boundaryDebugTargetMeta(dom.boundaryDebugTarget?.value||boundaryDebugTarget);if(!target)return false;boundaryDebugTarget=target.value;boundaryDebugTransforms[boundaryDebugStorageKey(target)]=boundaryDebugReadForm();boundaryDebugDrafts[target.value]={...boundaryDebugTransforms[boundaryDebugStorageKey(target)]};const ok=boundaryDebugPersist();boundaryDebugApplyTransforms(Number(dom.year.value));return ok}
...
function boundaryDebugSaveSnapshot(all=false){const y=Number(dom.year.value),currentTarget=boundaryDebugTargetMeta(dom.boundaryDebugTarget?.value||boundaryDebugTarget),elements=all?activeCountries():(()=>currentTarget?.type==="current"?activeCountries().filter(path=>path.dataset.id===currentTarget.id):[] )();if(!elements.length){if(dom.boundaryDebugStatus)dom.boundaryDebugStatus.textContent="当前对象不是地图上的现有疆域，或当前年份没有可保存的路径。";return}const transform=currentTarget?.type==="current"?boundaryDebugSettings(currentTarget,y):boundaryDebugDefaults,item={key:boundaryDebugId("saved"),id:all?`snapshot-${y}`:`${elements[0].dataset.id}-${y}`,name:all?`${y} 年全部疆域`:`${y} 年 · ${boundaryDebugName(elements[0].dataset.id,y)}`,year:y,source:"现有地图 SVG 快照",transform:{...transform},paths:boundaryDebugCapturePaths(elements)};boundaryDebugSaved.push(item);boundaryDebugTarget=`saved:${item.key}`;boundaryDebugPersist();boundaryDebugPopulateTargets(false);if(dom.boundaryDebugStatus)dom.boundaryDebugStatus.textContent=`已保存 ${item.name}，可在“已保存快照”中继续调试。`}
function boundaryDebugDeleteTarget(){const target=boundaryDebugTargetMeta(dom.boundaryDebugTarget?.value||boundaryDebugTarget);if(!target)return;if(target.type==="imported")boundaryDebugImported=boundaryDebugImported.filter(item=>item.key!==target.id);else if(target.type==="saved")boundaryDebugSaved=boundaryDebugSaved.filter(item=>item.key!==target.id);else{delete boundaryDebugTransforms[boundaryDebugStorageKey(target)];delete boundaryDebugDrafts[target.value]}boundaryDebugPersist();boundaryDebugTarget="";boundaryDebugPopulateTargets(false);boundaryDebugApplyTransforms(Number(dom.year.value));if(dom.boundaryDebugStatus)dom.boundaryDebugStatus.textContent=target.type==="current"?"已清除当前年份疆域的保存参数。":"已删除当前导入或快照。"}
function boundaryDebugResetTarget(){const target=boundaryDebugTargetMeta(dom.boundaryDebugTarget?.value||boundaryDebugTarget);if(!target)return;delete boundaryDebugDrafts[target.value];delete boundaryDebugTransforms[boundaryDebugStorageKey(target)];boundaryDebugPersist();boundaryDebugLoadForm();boundaryDebugApplyTransforms(Number(dom.year.value));if(dom.boundaryDebugStatus)dom.boundaryDebugStatus.textContent="已恢复默认位置与缩放。"}
function boundaryDebugRenderLayer(y){if(!dom.boundaryDebugLayer)return;dom.boundaryDebugLayer.innerHTML="";const target=boundaryDebugTargetMeta(dom.boundaryDebugTarget?.value||boundaryDebugTarget);if(!target)return;const ns="http://www.w3.org/2000/svg";if(target.type==="current"){const paths=activeCountries().filter(path=>path.dataset.id===target.id);paths.forEach(path=>{const outline=document.createElementNS(ns,"path");outline.classList.add("boundary-debug-reference");outline.setAttribute("d",path.getAttribute("d")||"");outline.setAttribute("fill-rule",path.getAttribute("fill-rule")||"nonzero");dom.boundaryDebugLayer.append(outline)});return}const item=boundaryDebugItemFor(target);if(!item)return;const settings=boundaryDebugSettings(target,y),group=document.createElementNS(ns,"g"),anchor=boundaryDebugPathRecordsBounds(item.paths);group.classList.add(target.type==="saved"?"boundary-debug-saved":"boundary-debug-outline");group.setAttribute("opacity",String(settings.opacity));group.setAttribute("transform",boundaryDebugTransform(anchor,settings));item.paths.forEach(record=>{const path=document.createElementNS(ns,"path");path.setAttribute("d",record.d);path.setAttribute("fill-rule",record.fillRule||"nonzero");group.append(path)});dom.boundaryDebugLayer.append(group)}
function boundaryDebugApplyTransforms(y){const current=activeCountries();current.forEach(path=>boundaryDebugResetElement(path));document.querySelectorAll(".flag-layer").forEach(boundaryDebugResetElement);const applied=new Set();current.forEach(path=>{const id=path.dataset.id;if(applied.has(id))return;const target={type:"current",id,value:`current:${id}`};if(!boundaryDebugHasCurrentOverride(target,y))return;const paths=current.filter(candidate=>candidate.dataset.id===id),settings=boundaryDebugSettings(target,y),anchor=boundaryDebugElementBounds(paths);paths.forEach(path=>boundaryDebugApplyElement(path,anchor,settings,settings.opacity===boundaryDebugDefaults.opacity?1:settings.opacity));document.querySelectorAll(`.flag-layer[data-for="${id}"]`).forEach(layer=>boundaryDebugApplyElement(layer,anchor,settings));applied.add(id)});boundaryDebugRenderLayer(y)}
async function boundaryDebugImportFile(){const file=dom.boundaryDebugFile?.files?.[0];if(!file)return;if(dom.boundaryDebugStatus)dom.boundaryDebugStatus.textContent="正在读取疆域文件…";try{const parsed=boundaryDebugParseImport(await file.text(),file.name);if(parsed.config){boundaryDebugPersist();boundaryDebugPopulateTargets(false);if(dom.boundaryDebugStatus)dom.boundaryDebugStatus.textContent="已载入调试配置。";return}if(!parsed.items.length)throw new Error("没有找到可用的 Feature、path 或 GeoJSON 几何");boundaryDebugImported.push(...parsed.items);boundaryDebugTarget=`imported:${parsed.items.at(-1).key}`;boundaryDebugPersist();boundaryDebugPopulateTargets(false);if(dom.boundaryDebugStatus)dom.boundaryDebugStatus.textContent=`已导入 ${parsed.items.length} 个疆域对象。`}catch(error){if(dom.boundaryDebugStatus)dom.boundaryDebugStatus.textContent=`导入失败：${error.message||error}`}}
async function boundaryDebugImportConfig(){const file=dom.boundaryDebugConfigFile?.files?.[0];if(!file)return;try{const value=JSON.parse(await file.text());if(!value||value.version!==1)throw new Error("不是本工具导出的版本 1 配置");boundaryDebugLoadStore(value);boundaryDebugDrafts={};boundaryDebugTarget="";boundaryDebugPersist();boundaryDebugPopulateTargets(false);if(dom.boundaryDebugStatus)dom.boundaryDebugStatus.textContent="已导入调试配置并保存到浏览器。"}catch(error){if(dom.boundaryDebugStatus)dom.boundaryDebugStatus.textContent=`配置导入失败：${error.message||error}`}}
function boundaryDebugExport(){const blob=new Blob([JSON.stringify(boundaryDebugStoreValue(),null,2)],{type:"application/json"}),url=URL.createObjectURL(blob),anchor=document.createElement("a");anchor.href=url;anchor.download="mena-boundary-debug.json";anchor.click();setTimeout(()=>URL.revokeObjectURL(url),0);if(dom.boundaryDebugStatus)dom.boundaryDebugStatus.textContent="已导出 mena-boundary-debug.json。"}
function boundaryDebugOpen(){boundaryDebugPopulateTargets(false);if(!dom.boundaryDebugDialog)return;dom.boundaryDebugDialog.showModal();boundaryDebugLoadForm()}
function boundaryDebugBind(){if(!dom.boundaryDebugButton)return;dom.boundaryDebugButton.addEventListener("click",boundaryDebugOpen);dom.boundaryDebugClose?.addEventListener("click",()=>dom.boundaryDebugDialog.close());dom.boundaryDebugDialog?.addEventListener("click",event=>{if(event.target===dom.boundaryDebugDialog)dom.boundaryDebugDialog.close()});dom.boundaryDebugTarget?.addEventListener("change",boundaryDebugLoadForm);[dom.boundaryDebugScaleX,dom.boundaryDebugScaleY,dom.boundaryDebugOffsetX,dom.boundaryDebugOffsetY,dom.boundaryDebugRotation,dom.boundaryDebugOpacity].forEach(input=>input?.addEventListener("input",boundaryDebugRefreshDraft));dom.boundaryDebugImport?.addEventListener("click",boundaryDebugImportFile);dom.boundaryDebugConfigImportButton?.addEventListener("click",()=>dom.boundaryDebugConfigFile?.click());dom.boundaryDebugConfigFile?.addEventListener("change",boundaryDebugImportConfig);dom.boundaryDebugSave?.addEventListener("click",()=>{const ok=boundaryDebugCommit();if(dom.boundaryDebugStatus)dom.boundaryDebugStatus.textContent=ok?"已保存调试参数到浏览器。":"浏览器存储不可用，已保留当前预览。"});dom.boundaryDebugSaveCurrent?.addEventListener("click",()=>boundaryDebugSaveSnapshot(false));dom.boundaryDebugSaveAll?.addEventListener("click",()=>boundaryDebugSaveSnapshot(true));dom.boundaryDebugReset?.addEventListener("click",boundaryDebugResetTarget);dom.boundaryDebugDelete?.addEventListener("click",boundaryDebugDeleteTarget);dom.boundaryDebugExport?.addEventListener("click",boundaryDebugExport)}
 boundaryDebugLoadStore();boundaryDebugBind();dom.schematicCountries.forEach(bindCountry);dom.year.addEventListener("input",()=>{stop();render();boundaryDebugPopulateTargets(true)});dom.play.addEventListener("click",toggle);dom.overview.addEventListener("click",overview);dom.brand.addEventListener("click",overview);dom.about.addEventListener("click",()=>dom.dialog.showModal());dom.close.addEventListener("click",()=>dom.dialog.close());dom.confirm.addEventListener("click",()=>dom.dialog.close());dom.dialog.addEventListener("click",e=>{if(e.target===dom.dialog)dom.dialog.close()});dom.jump.addEventListener("click",()=>{const raw=prompt("输入1797—2026之间的年份：",dom.year.value);if(raw===null)return;dom.year.value=Math.max(1797,Math.min(2026,parseInt(raw,10)||+dom.year.value));stop();render();boundaryDebugPopulateTargets(true)});render();
