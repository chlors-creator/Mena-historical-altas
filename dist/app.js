...
function boundaryFeaturesForYear(y){
  ...
  const selectedFeatures=sourceFeatures.filter(feature=>feature.from<=y&&feature.to>=y);
  ...
  const present=new Set(selectedFeatures.map(feature=>feature.id==="yemen-north"&&y>=1990?"yemen":feature.id));
