...
function flagGeometryFor(path){
  const d=path.getAttribute("d")||"",fillRule=path.getAttribute("fill-rule")||"nonzero";
  if(!d)return[];
  let box=null;
  try{box=path.getBBox()}catch{box=pathDataBox(d)}
  return box&&box.width>0&&box.height>0?[{d,box,fillRule}]:[];
}
