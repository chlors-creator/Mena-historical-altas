...
let flagDebugFileDefaults={};
const flagDebugFileDefaultsReady=typeof fetch==="function"?fetch("mena-flag-debug.json",{cache:"no-store"}).then(response=>response.ok?response.json():{}).then(value=>{if(value&&typeof value==="object")flagDebugFileDefaults=value}).catch(()=>{}):Promise.resolve();
function flagDebugSetFileDefaults(value){if(value&&typeof value==="object")flagDebugFileDefaults=value}
...
Object.assign(flagDebugBuiltIn,{
  ...
  "egypt::egypt-1952.svg":{scaleX:1.38,scaleY:1.4,offsetX:0,offsetY:0,rotation:0,color:"#a65d37",flagFill:"#00672b"},
...
function sharedColorFor(polity){if(!SHARED_POLITIES.has(polity))return"";const draft=flagDebugSharedDrafts[polity],saved=readFlagDebug();return draft||normalizeSharedColor(polity,saved.__sharedColors?.[polity])||normalizeSharedColor(polity,flagDebugFileDefaults.__sharedColors?.[polity])||SHARED_DEFAULT_COLORS[polity]||""}
function flagDebugSettings(id,y){const key=flagDebugKey(id,y),polity=polityAt(id,y),saved=readFlagDebug();let exact=saved[key];if(!exact&&saved[id]){exact=saved[id];const migrated={...saved,[key]:saved[id]};if(SHARED_POLITIES.has(polity)&&saved[id].color){migrated.__sharedColors={...(saved.__sharedColors||{}),[polity]:saved[id].color}}delete migrated[id];try{localStorage.setItem(FLAG_DEBUG_STORAGE,JSON.stringify(migrated))}catch{}}const builtIn={...(flagDebugBuiltIn[key]||{})},fileDefault={...(flagDebugFileDefaults[key]||{})};if(SHARED_POLITIES.has(polity)){delete builtIn.color;delete fileDefault.color}const settings={...flagDebugDefaults,...builtIn,...fileDefault,...(exact||{})};if(SHARED_POLITIES.has(polity))settings.color=sharedColorFor(polity)||"";return settings}
...
function clearFlagLayer(){document.querySelectorAll(".flag-layer").forEach(layer=>layer.remove());const defs=dom.map.querySelector("defs"),oldPatterns=defs?.querySelector("#flagPatterns"),oldMasks=defs?.querySelector("#flagMasks");if(oldPatterns)oldPatterns.remove();if(oldMasks)oldMasks.remove()}
function flagGeometryFor(path){const d=path.getAttribute("d")||"",cutoutCount=Number(path.dataset.cutoutCount)||0,fillRule=path.getAttribute("fill-rule")||"nonzero";if(cutoutCount||path.dataset.transform||path.dataset.sourceId==="hejaz"){const b=path.getBBox();return b.width&&b.height?[{d,box:b,fillRule}]:[]}return flagPathParts(d).map(part=>({d:part,box:pathDataBox(part),fillRule})).filter(item=>item.box&&item.box.width>0&&item.box.height>0)}
function flagBoundaryMaskFor(id,y,index,defs,ns){
  if(id!=="morocco"||Number(y)>=1912)return"";
  const masks=defs?.querySelector("#flagMasks"),cuts=activeCountries().filter(path=>path.dataset.id==="morocco-spanish");
  if(!masks||!cuts.length)return"";
  const mask=document.createElementNS(ns,"mask"),maskId=`activeFlagMask-${id}-${y}-${index}`;mask.id=maskId;mask.setAttribute("mask-type","luminance");mask.setAttribute("maskUnits","userSpaceOnUse");mask.setAttribute("maskContentUnits","userSpaceOnUse");mask.setAttribute("x","0");mask.setAttribute("y","0");mask.setAttribute("width","1080");mask.setAttribute("height","650");
  const white=document.createElementNS(ns,"rect");white.setAttribute("x","0");white.setAttribute("y","0");white.setAttribute("width","1080");white.setAttribute("height","650");white.setAttribute("fill","white");mask.append(white);
  cuts.forEach(path=>{const cut=document.createElementNS(ns,"path");cut.setAttribute("d",path.getAttribute("d")||"");cut.setAttribute("fill","black");cut.setAttribute("fill-rule",path.getAttribute("fill-rule")||"nonzero");if(path.dataset.transform)cut.setAttribute("transform",path.dataset.transform);mask.append(cut)});
  masks.append(mask);return`url(#${maskId})`
}
function createFlagLayer(id,y){const paths=activeCountries().filter(candidate=>candidate.dataset.id===id);if(!paths.length)return;const ns="http://www.w3.org/2000/svg",root=realMode?dom.realRoot:dom.schematicRoot,defs=dom.map.querySelector("defs"),patterns=document.createElementNS(ns,"g");patterns.setAttribute("id","flagPatterns");defs.append(patterns);const masks=document.createElementNS(ns,"g");masks.setAttribute("id","flagMasks");defs.append(masks);const layer=document.createElementNS(ns,"g"),asset=flagAssetFor(id,y);layer.classList.add("flag-layer");layer.dataset.for=id;layer.dataset.flagKey=flagDebugKey(id,y);layer.setAttribute("aria-hidden","true");if(id==="ottoman"&&paths.some(path=>path.dataset.cutoutPaths))layer.setAttribute("mask","url(#ottomanCutoutMask)");let geometryIndex=0;paths.forEach(path=>{flagGeometryFor(path).forEach(geometry=>{const index=geometryIndex++,clipId=`activeFlagClip-${id}-${index}`;const clip=document.createElementNS(ns,"clipPath");clip.setAttribute("id",clipId);clip.setAttribute("clipPathUnits","userSpaceOnUse");const clipPath=document.createElementNS(ns,"path");clipPath.setAttribute("d",geometry.d);clipPath.setAttribute("fill-rule",geometry.fillRule);clip.append(clipPath);patterns.append(clip);const flag=document.createElementNS(ns,"g");flag.classList.add("country-flag");flag.dataset.for=id;flag.dataset.part=String(index);flag.dataset.flagKey=flagDebugKey(id,y);flag.dataset.flagFile=asset.file;flag.dataset.flagPeriod=asset.period;flag.dataset.flagSource=asset.source;if(path.dataset.transform)flag.setAttribute("transform",path.dataset.transform);flag.setAttribute("clip-path",`url(#${clipId})`);const maskUrl=flagBoundaryMaskFor(id,y,index,defs,ns);if(maskUrl)flag.setAttribute("mask",maskUrl);const fill=document.createElementNS(ns,"path");fill.classList.add("flag-fill");fill.setAttribute("d",geometry.d);fill.setAttribute("fill-rule",geometry.fillRule);fill.setAttribute("fill","transparent");fill.style.setProperty("fill","transparent","important");fill.setAttribute("pointer-events","none");flag.append(fill);const image=document.createElementNS(ns,"image"),b=geometry.box;image.setAttribute("x",b.x);image.setAttribute("y",b.y);image.setAttribute("width",Math.max(2,b.width));image.setAttribute("height",Math.max(2,b.height));image.dataset.baseX=String(b.x);image.dataset.baseY=String(b.y);image.dataset.baseWidth=String(Math.max(2,b.width));image.dataset.baseHeight=String(Math.max(2,b.height));image.setAttribute("preserveAspectRatio","xMidYMid slice");image.setAttribute("href",asset.path);image.setAttributeNS("http://www.w3.org/1999/xlink","href",asset.path);image.setAttribute("clip-path",`url(#${clipId})`);image.setAttribute("role","presentation");flag.append(image);layer.append(flag)})});root.append(layer);applyFlagDebugSettings(id,y);requestAnimationFrame(()=>{if(!layer.isConnected||activeFlagId!==id)return;void layer.getBoundingClientRect();layer.querySelectorAll(".country-flag").forEach(flag=>flag.classList.add("active-flag"))})}
