...
function flagDebugSettings(id,y){const key=flagDebugKey(id,y),saved=readFlagDebug();let exact=saved[key];if(!exact&&saved[id]){exact=saved[id];const migrated={...saved,[key]:saved[id]};delete migrated[id];try{localStorage.setItem(FLAG_DEBUG_STORAGE,JSON.stringify(migrated))}catch{}}return {...flagDebugDefaults,...(flagDebugBuiltIn[key]||{}),...(exact||{})}}
...
const _renderWithFormalNames=render;render=()=>{_renderWithFormalNames();const y=+dom.year.value;updateFlagDebugPreview(selected,y);if(dom.flagDebugButton)dom.flagDebugButton.hidden=!selected;activeCountries().forEach(path=>{const label=displayNameAt(path.dataset.id,y);path.setAttribute("aria-label",`查看${label}`)});document.querySelectorAll(".map-labels text[data-for]").forEach(label=>{label.textContent=displayNameAt(label.dataset.for,y)});if(!selected)return;const display=displayNameAt(selected,y);dom.selectedLabel.textContent=display;dom.polityName.textContent=formalNameAt(selected,y);dom.polityNative.textContent=display;const firstFact=dom.facts.querySelector("div strong");if(firstFact){firstFact.textContent=rulingGroupAt(selected,y);const firstLabel=firstFact.previousElementSibling;if(firstLabel)firstLabel.textContent="执政党／统治集团"}};
 ...
 function refreshDebugDraft(){const id=selected;if(!id)return;const y=Number(dom.year.value),key=flagDebugKey(id,y);flagDebugDrafts[key]=debugFormValues();updateDebugOutputs();applyFlagDebugSettings(id,y);dom.flagDebugStatus.textContent="实时预览中；点击“保存到浏览器”后将在本机保留。"}
 function saveDebugSettings(){const id=selected;if(!id)return;const y=Number(dom.year.value),key=flagDebugKey(id,y),saved=readFlagDebug();saved[key]={...flagDebugSettings(id,y),...debugFormValues()};delete saved[id];try{localStorage.setItem(FLAG_DEBUG_STORAGE,JSON.stringify(saved,null,2));flagDebugDrafts[key]={...saved[key]};dom.flagDebugStatus.textContent="已保存到本浏览器。"}catch{dom.flagDebugStatus.textContent="浏览器存储不可用，请使用导出 JSON。"}applyFlagDebugSettings(id,y)}
 function resetDebugSettings(){const id=selected;if(!id)return;const y=Number(dom.year.value),key=flagDebugKey(id,y);delete flagDebugDrafts[key];const saved=readFlagDebug();delete saved[key];delete saved[id];try{localStorage.setItem(FLAG_DEBUG_STORAGE,JSON.stringify(saved))}catch{}historicalStyles(y);syncFlags(y);loadDebugForm(id);dom.flagDebugStatus.textContent="已恢复默认设置。"}
